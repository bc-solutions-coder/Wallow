# Foundry Platform Analysis & Strategic Design
...
## 1. Verdict: Is Foundry a Solid Generic Base?
...
Foundry is ready to fork into your first product. The fixes below are improvements, not blockers.
...
**Fix:** Create `TenantAwareDbContext<TContext>` base class in `Foundry.Shared.Infrastructure`:
...
Add to `FoundryModules.cs` so forks control modules via `appsettings.json`:
...
**Setup (one-time):**
1. Fork `foundry` on GitHub
2. `git remote add upstream git@github.com:you/foundry.git`
...
**Contributing back:**
- Build feature in fork, refine it, then re-implement generically in Foundry via PR
- Convention: prefix upstream-intended commits with `[foundry]` for easy identification

**Product-specific modules:** Implement as Foundry plugins (using existing plugin system) so they don't conflict during upstream rebases.
...
For your workflow of "build in fork, then genericize back to Foundry":

1. **In the fork project**, build the feature with product-specific needs
2. **When ready to genericize**, ask Claude in the Foundry project:
...
4. Implementation happens in Foundry with proper tests
