...
public sealed partial class IdentityDataSeeder(ILogger<IdentityDataSeeder> logger)
{
    ...
    private async Task SeedRolesAsync(RoleManager<FoundryRole> roleManager)
    {
    ...
    private async Task SeedAdminUserAsync(
        ...
        TimeProvider timeProvider)
    {
        const string adminEmail = "admin@foundry.dev";
        const string adminPassword = "Admin123!";

        FoundryUser? existingUser = await userManager.FindByEmailAsync(adminEmail);
        if (existingUser is not null)
        {
            return;
        }

        FoundryUser adminUser = FoundryUser.Create(
            tenantId: Guid.Empty,
            firstName: "Admin",
            lastName: "User",
            email: adminEmail,
    ...
    {
        const string clientId = "foundry-dev-client";
        ...
        {
            ClientId = clientId,
            DisplayName = "Foundry Dev Client",
