...
- `AddFoundryMarten()` - Configures shared IDocumentStore
...
- `FoundryModules.cs` - Central module registration
...
### Marten Setup
```csharp
builder.Services.AddFoundryMarten(builder.Configuration);

// Requires appsettings.json:
{
  "ConnectionStrings": {
    "Foundry": "Host=localhost;Database=foundry;..."
  }
}
```
...
- Register modules in FoundryModules.cs

## NuGet Potential

**Low** - Tightly coupled to Foundry's architecture decisions (Marten + Wolverine + PostgreSQL).
