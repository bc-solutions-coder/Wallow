...
- **Must not** reference `Foundry.Shared.Contracts` (Kernel is lower-level than Contracts).
...
- **Depended on by**: Every module's Domain layer, plus Application and Infrastructure layers. Also referenced by `Foundry.Api`.
...
- Do not add integration event types here. Those belong in `Foundry.Shared.Contracts`.
