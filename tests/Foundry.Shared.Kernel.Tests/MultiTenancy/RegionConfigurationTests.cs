...
{
    [Fact]
    public void US_EAST_HasExpectedValue()
    {
        RegionConfiguration.US_EAST.Should().Be("us-east-1");
    }

    [Fact]
    public void EU_WEST_HasExpectedValue()
    {
        RegionConfiguration.EU_WEST.Should().Be("eu-west-1");
    }

    [Fact]
    public void AP_SOUTHEAST_HasExpectedValue()
    {
        RegionConfiguration.AP_SOUTHEAST.Should().Be("ap-southeast-1");
    }

    [Fact]
    public void PrimaryRegion_IsUsEast()
    {
        RegionConfiguration.PrimaryRegion.Should().Be(RegionConfiguration.US_EAST);
    }

    [Fact]
    public void AllRegionConstants_AreUnique()
    {
        string[] regions = [RegionConfiguration.US_EAST, RegionConfiguration.EU_WEST, RegionConfiguration.AP_SOUTHEAST];
