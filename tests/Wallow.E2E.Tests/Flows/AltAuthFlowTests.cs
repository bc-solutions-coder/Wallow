...
public sealed class AltAuthFlowTests : E2ETestBase
{
    ...
    public async Task MagicLinkLogin_HappyPath_LandsDashboard()
    {
        ...
        string magicLink = await MailpitHelper.SearchForLinkAsync(
            Docker.MailpitBaseUrl, user.Email, "magic-link");

        if (string.IsNullOrEmpty(magicLink))
        {
            magicLink = await MailpitHelper.SearchForLinkAsync(
                Docker.MailpitBaseUrl, user.Email, "passwordless", maxRetries: 3, pollIntervalSeconds: 1);
        }
